...
Deno.test("SQLite - Patient Repository", async (t) => {
	...
	await t.step("Deve retornar false se o paciente não existir.", async () => {
		const db = await init_test_db();
		populate(db);
		const patientId = ID.fromString("some-fake-id");
		const repository = new SQLitePatientRepository(db);
	...
	await t.step("Deve atualizar os dados de um paciente.", async () => {
		...
		const patientOrError = await repository.getById(patient1.systemId);
	...
	await t.step(
		...
		async () => {
			...
			const patientOrError = await repository.getById(ID.fromString("some-fake-id"));
	...
	await t.step("Deve recuperar o Dono do paciente.", async () => {
		...
		const ownerOrError = await repository.findOwner(ID.fromString("1001"));
	...
	await t.step(
		...
		async () => {
			...
			const ownerOrError = await repository.findOwner(ID.fromString("some-fake-id"));
