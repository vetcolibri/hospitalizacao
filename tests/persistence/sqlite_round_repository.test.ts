...
Deno.test("SQLite - Round Repository", async (t) => {
    await t.step("Deve salvar uma ronda com o parametro da frequência cardiaca", async () => {
    ...
    await t.step("Deve recuperar as ultimas medições do paciente para cada parametro", async () => {
        const db = await init_test_db()
        populate(db)
        const persistence = new SQLiteRoundRepository(db)
        const round = new Round(patient1)
        ...
        const measurements = await persistence.latestMeasurements(patient1.patientId)
