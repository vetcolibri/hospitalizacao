...
Deno.test("Patient Service - Hospitalizad Patients", async (t) => {
  ...
  await t.step(
    "Deve buscar no repositório os pacientes hospitalzados.",
    async () => {
...
Deno.test("Patient Service - New Hospitalization", async (t) => {
  ...
  await t.step(
    "Deve registar o peso do paciente a hospitalização·",
    async () => {
      const { service, patientRepository } = makeService({
        patientRepository: new PatientRepositoryStub(),
      });
...
Deno.test("Patient Service - Non Hospitalized Patients", async (t) => {
  await t.step(
    "Deve chamar o método nonHospitalized no repositório",
    async () => {
      const { service, patientRepository } = makeService();
...
Deno.test("Patient Service - New Patient", async (t) => {
  ...
  await t.step(
    "Deve verificar se ID do paciente já foi registrado.",
    async () => {
      const { service, patientRepository } = makeService();
      ...
      assertSpyCall(repoSpy, 0, {
        args: [ID.New(newPatientData.patientData.patientId)],
      });
      assertSpyCalls(repoSpy, 1);
    }
  );
  ...
  await t.step(
    "Deve retonar @IDAlreadyExists se ID do paciente já foi registrado.",
    async () => {
      const { service } = makeService({
        patientRepository: new PatientRepositoryStub(),
      });
...
function makeService(options?: Options) {
  const patientRepository =
    options?.patientRepository ?? new InmemPatientRepository();
