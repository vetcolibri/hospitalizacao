...
function makeService(budgets: Budget[], hospitalizations: Hospitalization[]) {
	...
	return { service, budgetRepository, patientRepository };
}
...
Deno.test("RF-16 - orçamento de episódio encerrado é só de leitura", async (t) => {
	...
	await t.step("bloqueia o paciente antes de validar o episódio", async () => {
		...
		await service.update("b-h2", { startOn: "2026-02-01", endOn: "2026-02-10" }, MEDVET);
