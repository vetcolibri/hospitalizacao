...
const patient = Patient.builder()
	.withPatientId(ID.fromString(patientIdData.patientIdId))
	.withName(patientIdData.name)
	.withSpecie(patientIdData.specie as Specie)
	.withBreed(patientIdData.breed)
	.withBirthDate(new BirthDate(patientIdData.birthDate))
	.withOwnerId(ID.fromString("1001"))
	.build().right;
