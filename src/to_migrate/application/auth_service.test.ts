...
import type { TokenGenerator } from "@domain/auth/token_generator.ts";
...
import { assertEquals, assertInstanceOf, assertObjectMatch } from "@deps/assert";

import { assertSpyCall, assertSpyCallArg, assertSpyCalls, spy } from "@deps/mock";
import { TokenGeneratorStub } from "../test_doubles/token_generator_stub.ts";
...
Deno.test("Auth Service - Login", async (t) => {
	...
	await t.step(
		"Deve retornar **AuthenticationFailed** quando o utilizador não for encontrado",
		async () => {
			const { service } = makeService();
	...
	await t.step(
		"Deve retornar os dados do utilizador quando o login foi bem sucedido",
		async () => {
			const { service } = makeService();
