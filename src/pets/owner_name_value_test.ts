...
Deno.test("OwnerNameValue", async (t) => {
	await t.step("Deve retornar um nome do tutor válido se tiver pelo menos um nome e um sobrenome", () => {
		const result = OwnerNameValue.fromString("John Doe");
		assertEquals(result.isRight(), true);
	});

	await t.step("Deve retornar um erro se o nome do tutor não tiver pelo menos um nome e um sobrenome", () => {
