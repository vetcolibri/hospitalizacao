...
Deno.test("HospitalizationsService.createHospitalization", async (t) => {
	...
	await t.step("Deve retornar erro se o pet já tiver uma hospitalização ativa", async () => {
		...
		const hospitalization = Hospitalization.create(
			IdValue.random(),
			DateValue.fromString("2024-01-01").right,
			DateValue.fromString("2024-01-03").right,
			[DiagnosisEnum.OTHER_INFECTION],
			[ComplaintEnum.FEVER, ComplaintEnum.LETHARGY],
			petId,
			"Luna",
			"1 ano",
			8.2,
...
Deno.test("HospitalizationsService.updateContactPerson", async (t) => {
	await t.step("Deve atualizar a pessoa de contacto com sucesso", async () => {
		...
		const hospitalization = Hospitalization.create(
			...
			DateValue.fromString("2024-01-03").right,
			[DiagnosisEnum.OTHER_INFECTION],
			[ComplaintEnum.OTHER], // Or a more specific enum member if appropriate
			IdValue.random(),
			"PetName",
			"1 ano",
			5,
...
Deno.test("HospitalizationsService.updateDiagnosis", async (t) => {
	await t.step("Deve atualizar o diagnóstico com sucesso", async () => {
		...
		const hospitalization = Hospitalization.create(
			...
			DateValue.fromString("2024-01-01").right,
			DateValue.fromString("2024-01-03").right,
			[DiagnosisEnum.GASTROENTERITIS, DiagnosisEnum.DEHYDRATION],
			[ComplaintEnum.VOMITING, ComplaintEnum.DIARRHEA],
			IdValue.random(),
			"PetName",
			"1 ano",
			5,
	...
	await t.step(
		...
		async () => {
			...
			const hospitalization = Hospitalization.create(
				...
				DateValue.fromString("2024-01-01").right,
				DateValue.fromString("2024-01-03").right,
				[DiagnosisEnum.DEHYDRATION],
				[ComplaintEnum.LETHARGY],
				IdValue.random(),
				"Pet Name",
				"1",
				1,
...
Deno.test("HospitalizationsService.dischargeHospitalization", async (t) => {
	await t.step("Deve dar alta a uma hospitalização com sucesso", async () => {
		...
		const hospitalization = Hospitalization.create(
			...
			DateValue.fromString("2024-01-01").right,
			DateValue.fromString("2024-01-03").right,
			[DiagnosisEnum.POST_SURGICAL_OBSERVATION],
			[ComplaintEnum.PAIN],
			IdValue.random(),
			"Bella",
			"1 ano",
...
Deno.test("HospitalizationsService.createPeriodicReport", async (t) => {
	await t.step("Deve criar um relatório periódico com sucesso e publicar evento", async () => {
		...
		const hospitalization = Hospitalization.create(
			...
			DateValue.fromString("2024-01-01").right,
			DateValue.fromString("2024-01-03").right,
			[DiagnosisEnum.GASTROENTERITIS],
			[ComplaintEnum.VOMITING],
			IdValue.random(),
			"Buddy",
			"3 anos",
			15.5,
	...
	await t.step("Deve criar um relatório simples sem alimentação nem descargas e publicar evento", async () => {
		...
		const hospitalization = Hospitalization.create(
			...
			DateValue.fromString("2024-01-01").right,
			DateValue.fromString("2024-01-03").right,
			[DiagnosisEnum.KENNEL_COUGH],
			[ComplaintEnum.COUGHING],
			IdValue.random(),
			"Rex",
			"5 anos",
			25.0,
