...
function toReportDTO(row: RowObject): ReportDTO {
    return {
        ...
        food: {
            types: JSON.parse(String(row.food_type)).split(","),
            level: String(row.food_level),
            datetime: String(row.food_date),
        },
        discharge: {
            type: String(row.discharge_type),
            aspect: String(row.discharge_aspect),
        },
...
export class SQLiteReportService implements ReportService {
    ...
    getAll(patientId: string, hospitalizationId: string): Promise<ReportDTO[]> {
        const rows = this.#db.queryEntries(
            "SELECT b.status as b_status, patients.name, patients.patient_id, owners.owner_name, hospitalizations.system_id, reports.report_id, reports.state_of_consciousness, reports.food_type, reports.food_level, reports.food_date, reports.discharge_type, reports.discharge_aspect, reports.created_at, reports.comments FROM budgets b INNER JOIN hospitalizations ON b.hospitalization_id = :hospitalizationId INNER JOIN reports ON hospitalizations.system_id = reports.system_id INNER JOIN patients ON reports.system_id = patients.system_id INNER JOIN owners ON patients.owner_id = owners.owner_id WHERE reports.system_id = :patientId",
